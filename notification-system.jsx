...
/**
 * NEW HORIZON — Notification System
 * Covers: In-app notification center, real-time badge updates,
 * push notification registration, email templates, and notification preferences.
 *
 * Integrates with: Supabase real-time, Expo Push, Resend (email)
 */
...
// ─── MOCK DATA (replace with NotificationService from database-service.js) ─────
const MOCK_NOTIFS = [
  { id:"n1", type:"match",   title:"It's a Match! 🎉", body:"You and Alicia Rivera both liked each other. Say hello!", icon:"♥", time:"Just now",   read:false, actionUrl:"/messages/u2" },
  { id:"n2", type:"message", title:"New Message",       body:"Devon W. sent you a message: 'Brother, faith is what kept me...'", icon:"✉", time:"5m ago",   read:false, actionUrl:"/messages/u3" },
  ...
  { id:"n5", type:"resource",title:"New Resource Added",body:"A new expungement guide for Texas residents was posted in Resources.", icon:"📋", time:"1d ago",   read:true,  actionUrl:"/resources" },
  { id:"n6", type:"match",   title:"Someone Liked You!",body:"A member from New York liked your profile. Check out who it might be.", icon:"♡", time:"1d ago",   read:true,  actionUrl:"/connect" },
  { id:"n7", type:"job",     title:"Application Update",body:"Fresh Start Kitchens viewed your application for Culinary Assistant.", icon:"👁", time:"2d ago",   read:true,  actionUrl:"/jobs" },
  { id:"n8", type:"system",  title:"Welcome to New Horizon", body:"Your account is active. Start by completing your profile for more connections.", icon:"✦", time:"3d ago",   read:true,  actionUrl:"/profile" },
];
...
function NotificationCenter() {
  ...
  const [tab, setTab] = useState("center"); // center | prefs | templates
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      const newN = { id:"live_1", type:"match", title:"New Match Alert!", body:"Keisha Monroe just liked your profile.", icon:"♥", time:"Now", read:false, actionUrl:"/connect" };
      setIncoming(newN);
      setTimeout(() => { setIncoming(null); setNotifs(prev => [newN, ...prev]); }, 4500);
    }, 6000);
    return () => clearTimeout(t);
  }, []);
    ...
    <div style={{ minHeight:"100vh", background:T.ivory, fontFamily:"'DM Sans', sans-serif" }}>
      ...
      {incoming && (
        <div style={{
          ...
          width:320, animation:"slideDown .3s ease",
          ...
          <div ref={bellRef} style={{ position:"relative" }}>
            ...
            {showBellMenu && (
              <div style={{ position:"absolute", right:0, top:"100%", marginTop:8, width:360, background:"white", borderRadius:16, border:`1px solid ${T.mist}`, boxShadow:"0 16px 48px rgba(0,0,0,.14)", overflow:"hidden", animation:"slideDown .25s ease", zIndex:200 }}>
                ...
                <div style={{ padding:"10px 18px", borderTop:`1px solid ${T.mist}`, textAlign:"center" }}>
                  <button onClick={() => setShowBellMenu(false)} style={{ background:"none", color:T.gold, fontSize:13, fontWeight:500 }}>See all notifications →</button>
                </div>
              </div>
            )}
          </div>
      ...
      <div style={{ maxWidth:1000, margin:"0 auto", padding:"32px 24px" }}>
        <div style={{ marginBottom:28 }}>
          ...
          <p style={{ color:T.slate }}>Real-time alerts, preferences, and email templates — all managed here.</p>
        </div>
            ...
            <div>
              ...
              <div style={{ background:`linear-gradient(135deg,${T.charcoal},#2A2A2E)`, borderRadius:16, padding:20 }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                  ...
                  <span style={{ color:"white", fontSize:13, fontWeight:500 }}>WebSocket Connected</span>
                </div>
                <div style={{ fontSize:12, color:"#7A7A8A", lineHeight:1.6 }}>
                  Listening to Supabase real-time channel. New notifications appear instantly without page refresh.
                </div>
              </div>
            </div>
          </div>
        ...
        {tab === "prefs" && (
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
            ...
            <div style={{ gridColumn:"1/-1", background:"white", borderRadius:16, border:`1px solid ${T.mist}`, padding:24 }}>
              ...
              <p style={{ color:T.slate, fontSize:13, marginBottom:16 }}>No push notifications will be sent during these hours.</p>
              <div style={{ display:"flex", gap:16, flexWrap:"wrap" }}>
                {[["Start Time","22:00"],["End Time","07:00"],["Timezone","America/Chicago"]].map(([l,d]) => (
                  <div key={l}>
              ...
              <button style={{ marginTop:16, padding:"10px 24px", background:`linear-gradient(135deg,${T.gold},${T.goldL})`, color:"white", borderRadius:8, fontSize:14, fontWeight:500 }}>Save Preferences</button>
            </div>
          </div>
        )}
        ...
        {tab === "templates" && (
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            {[
              { name:"Welcome Email", trigger:"On registration", subject:"Welcome to New Horizon ✦", preview:"You've taken a powerful first step...", status:"Active" },
              { name:"Match Notification", trigger:"On mutual like", subject:"You have a new match!", preview:"Someone in your area liked you back...", status:"Active" },
              { name:"New Message Alert", trigger:"On message received", subject:"You have a new message", preview:"{{sender_name}} sent you a message...", status:"Active" },
              { name:"Job Alert Digest", trigger:"Daily at 9AM", subject:"{{job_count}} new jobs match your profile", preview:"Here are today's felony-friendly opportunities...", status:"Active" },
              { name:"Weekly Resource Update", trigger:"Every Monday", subject:"New resources for returning citizens in {{state}}", preview:"We've added new guides and tools this week...", status:"Draft" },
              { name:"Account Verification", trigger:"On signup", subject:"Verify your New Horizon email", preview:"Click the link below to confirm your account...", status:"Active" },
            ].map((t, i) => (
              <div key={i} style={{ background:"white", borderRadius:14, border:`1px solid ${T.mist}`, padding:"18px 22px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:14, animation:`fadeUp .4s ${i*.06}s ease both` }}>
