 ...
 * React Native + Expo
 *
 * Self-contained demo: local mock data, no backend required.
 * To wire up the live backend, replace `mockDB` with Supabase calls
 * (@supabase/supabase-js) and `registerForPushNotifications` with
 * expo-notifications — the same schema the web apps use.
 */
...
// ─── DATABASE CLIENT (mock — swap for Supabase in production) ───────────────────
const mockDB = {
  async getUser(id) {
...
function HomeTab({ user, onNavigate }) {
  ...
  const quickActions = [
    ...
    { icon: '🔔', label: 'Alerts', tab: 'Notifications', color: T.info },
    { icon: '◯', label: 'Profile', tab: 'Profile', color: T.gold },
  ];
      ...
      <FlatList ref={scrollRef} data={messages} keyExtractor={m => m.id}
        ...
        renderItem={({ item: m }) => (
          <View style={{ flexDirection: 'row', justifyContent: m.from === user.id ? 'flex-end' : 'flex-start', marginBottom: 6 }}>
            <View style={{ backgroundColor: m.from === user.id ? T.gold : T.white, padding: 12, borderRadius: 18, borderBottomRightRadius: m.from === user.id ? 4 : 18, borderBottomLeftRadius: m.from === user.id ? 18 : 4, maxWidth: W * 0.7, borderWidth: m.from === user.id ? 0 : 1, borderColor: T.mist }}>
